...
struct ARTreasureView: UIViewRepresentable {
    ...
    class Coordinator: NSObject, ARSCNViewDelegate, ARSessionDelegate, ARCoachingOverlayViewDelegate {
        ...
        func createTreasureNode(for treasure: Treasure) -> SCNNode {
            ...
            if let image = UIImage(systemName: treasure.symbolName, withConfiguration: symbolConfig) {
                let plane = SCNPlane(width: 0.3, height: 0.3)
                plane.firstMaterial?.diffuse.contents = image.withTintColor(.systemYellow, renderingMode: .alwaysOriginal)
            ...
            let text = SCNText(string: treasure.name, extrusionDepth: 0.01)
